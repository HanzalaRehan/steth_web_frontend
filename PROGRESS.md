...
**`npm run build` and a live browser click-through were not completed this session** — this sandbox's disk I/O was severely degraded this time (measured directly: raw sequential reads off `src/assets/` ran at roughly 380KB/s, versus the fast, unremarkable I/O every earlier session in this project reported). `npm run build`, a direct `vite build`, and `npm run dev` all sat at native, near-zero CPU usage for 10+ minutes without completing or erroring — confirmed via `ps` CPU-time deltas and process `lsof` (the process was genuinely blocked reading real project files, not hung on a network call or deadlocked). This is a sandbox-instance characteristic, not a code defect — every file this session touched passed lint cleanly and was manually re-read after editing to confirm correctness, but the production bundle was never actually produced and the megamenu/footer/checkout UI was never actually rendered in a browser this session. Retry `npm run build` and a real click-through (megamenu hover/accordion, footer link resolution, gift-card apply/remove) before treating this as done — this is a real gap, not a formality.

### What's next
Live-verify: `npm run build`, then a real browser pass — megamenu (desktop hover + mobile 375px accordion) showing live Fabric/Category/Color data and actually filtering `/women`/`/men` on click; every footer link resolving; gift-card purchase → code display → checkout apply/remove → order submit, ideally against a real dev/staging backend (this dev environment has no `.env.local`, so `API_BASE_URL` falls back to production — same standing gap noted in the prior Auth/UX session). Blog admin CRUD screen and Affiliate's real request-flow are both intentionally deferred, tracked in the backend's PROGRESS.md. Real Facebook/TikTok URLs whenever available (currently placeholder `#` hrefs, confirmed with you).
