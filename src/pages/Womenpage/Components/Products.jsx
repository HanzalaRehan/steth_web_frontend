...
// Category inference from product name - fallback for products with no
// `category` set server-side (Men's page doesn't need this fallback since
// its catalog has category set directly; kept here since it was already
// this page's own pre-existing behavior).
const inferCategory = (product) => {
  if (product.category) return product.category
  const nameLower = product.name.toLowerCase()
  if (nameLower.includes('coat')) return 'Lab Coats'
  if (nameLower.includes('cap')) return 'Caps'
  if (nameLower.includes('mask')) return 'Masks'
  if (nameLower.includes('bottle')) return 'Bottles'
  return 'Scrubs'
}
...
export default function ProductPage() {
  ...
  useEffect(() => {
    const fetchFilterOptions = async () => {
      try {
        const params = new URLSearchParams({ gender: "Women", limit: String(OPTIONS_FETCH_LIMIT) })
  ...
  return (
    <div id="products" className="mx-auto px-4 lg:px-20 py-8 w-full">
      {/* Heading */}
      <h1 className="text-2xl md:text-3xl lg:text-4xl font-bold mb-6 text-black px-4">Women's Collection</h1>
